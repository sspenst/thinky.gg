...
export async function getUserFromToken(
  ...
  const [user, userConfig] = await Promise.all([
    ...
    UserConfigModel.findOne({ userId: userId, gameId: gameId }, { gameId: 1, lastPlayedAt: 1, calcRankedSolves: 1, calcLevelsCreatedCount: 1, calcLevelsSolvedCount: 1, chapterUnlocked: 1, roles: 1 }).lean<UserConfig>(),
  ]);
