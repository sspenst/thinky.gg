...
export async function getLatestLevels(reqUser: User | null = null) {
  ...
  try {
    const levels = await LevelModel.find<Level>({ isDraft: false }, '_id slug leastMoves name userId ts points calc_playattempts_unique_users calc_playattempts_duration_sum calc_playattempts_just_beaten_count', { lean: false })
      .populate('userId')
      .sort({ ts: -1 })
      .limit(10);
