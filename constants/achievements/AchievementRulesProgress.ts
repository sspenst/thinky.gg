...
const AchievementRulesProgress: { [achievementType: string]: IAchievementInfoUser; } = {
  [AchievementType.SOLVED_LEVELS_5000]: {
    ...
    unlocked: ({ user }) => (user.config?.calcLevelsSolvedCount || 0) >= 5000,
  },
  [AchievementType.SOLVED_LEVELS_4000]: {
    ...
    discordNotification: true,
    unlocked: ({ user }) => (user.config?.calcLevelsSolvedCount || 0) >= 4000,
  },
  [AchievementType.SOLVED_LEVELS_3000]: {
    ...
    discordNotification: true,
    unlocked: ({ user }) => (user.config?.calcLevelsSolvedCount || 0) >= 3000,
  },
  [AchievementType.SOLVED_LEVELS_2000]: {
    ...
    discordNotification: true,
    unlocked: ({ user }) => (user.config?.calcLevelsSolvedCount || 0) >= 2000,
  },
  [AchievementType.SOLVED_LEVELS_1000]: {
    ...
    discordNotification: true,
    unlocked: ({ user }) => (user.config?.calcLevelsSolvedCount || 0) >= 1000,
  },
  [AchievementType.SOLVED_LEVELS_500]: {
    name: 'We\'re serious',
    emoji: '🏆',
    description: 'Solved 500 levels',
    unlocked: ({ user }) => (user.config?.calcLevelsSolvedCount || 0) >= 500,
  },
  [AchievementType.SOLVED_LEVELS_100]: {
    name: 'Getting Started',
    emoji: '🏁',
    description: 'Solved 100 levels',
    unlocked: ({ user }) => (user.config?.calcLevelsSolvedCount || 0) >= 100,
  },
};
