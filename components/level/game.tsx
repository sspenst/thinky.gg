...
export default function Game({ level, mutateLevel, onComplete, onNext }: GameProps) {
  ...
  useEffect(() => {
    const _controls = [
      new Control('btn-restart', () => handleKeyDown('KeyR'),
        <>
      ...
      new Control('btn-undo', () => handleKeyDown('Backspace'),
        <>
          <svg xmlns='http://www.w3.org/2000/svg' className='h-7 w-7' fill='none' viewBox='0 0 24 24' stroke='currentColor' strokeWidth={2} style={{ display: 'inline-block' }}>
    ...
    if (onNext) {
      _controls.push(new Control('btn-next', () => onNext(),
        <>
          <svg xmlns='http://www.w3.org/2000/svg' className='h-6 w-6' fill='none' viewBox='0 0 24 24' stroke='currentColor' strokeWidth={2} style={{ display: 'inline-block' }}>
