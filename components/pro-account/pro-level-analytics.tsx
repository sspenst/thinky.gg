    ...
    <table className='table-auto'>
      ...
      <tbody>
        ...
        <tr key={'prostat-playattemptgraph-total'}>
          ...
          <td key={'total-sum'} className='border px-4 py-2'>{moment.duration(prostats?.playAttemptData.reduce((a, b) => a + b.sum, 0), 'minutes').asHours().toFixed(1)}h</td>
        </tr>
      </tbody>
    ...
    <div className='w-full'>
      <ResponsiveContainer width='100%' height={300}>
        ...
        >
          ...
          <YAxis
            type='number'
            tickFormatter={(sum) => moment.duration(sum, 'minutes').asHours().toFixed(1) + 'h'}
          />
          <Tooltip
            cursor={false}
            content={
              ({ active, payload }) => {
                if (active && payload && payload.length) {
                  ...
                  const display = moment.duration(payloadObj.sum, 'minutes').asHours().toFixed(1) + 'h';
