...
import UniversePage, { getServerSideProps, UniversePageProps } from '../../../pages/universe/[id]';
...
describe('pages/universe', () => {
  test('getServerSideProps with no query', async () => {
    const resp: any = await getServerSideProps({ params: { id: TestId.USER }, query: {} } as unknown as GetServerSidePropsContext);
  ...
  test('getServerSideProps with search query', async () => {
    const resp: any = await getServerSideProps({ params: { id: TestId.USER }, query: { search: 'talll' } } as unknown as GetServerSidePropsContext);
    const props = resp.props as UniversePageProps;
  ...
  test('getServerSideProps with invalid object id should fail', async () => {
    const resp: any = await getServerSideProps({ params: { id: new ObjectId() } } as unknown as GetServerSidePropsContext);

    expect(resp?.notFound).toBe(true);
  });
  test('getServerSideProps with wrong id should fail', async () => {
    const resp: any = await getServerSideProps({ params: { id: 'wrong' } } as unknown as GetServerSidePropsContext);
