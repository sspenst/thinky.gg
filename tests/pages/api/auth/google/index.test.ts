...
import handler from '../../../../../pages/api/auth/google/index';
...
describe('pages/api/auth/google/index.ts', () => {
  ...
  describe('OAuth Initiation', () => {
    test('Should redirect to Google authorization URL with correct parameters', async () => {
    ...
    test('Should return 500 if Google client ID is not configured', async () => {
      ...
      await testApiHandler({
        ...
        test: async ({ fetch }) => {
          ...
          expect(response.error).toBe('Google client ID not configured');
        },
      });
    ...
    test('Should use environment GOOGLE_REDIRECT_URI when available', async () => {
      const originalRedirectUri = process.env.GOOGLE_REDIRECT_URI;

      process.env.GOOGLE_REDIRECT_URI = 'https://custom.com/auth/google/callback';
      ...
      await testApiHandler({
        ...
        test: async ({ fetch }) => {
          ...
          expect(url.searchParams.get('redirect_uri')).toBe('https://custom.com/auth/google/callback');
        },
      });
    ...
    test('Should construct redirect URI from request origin when GOOGLE_REDIRECT_URI is not set', async () => {
      const originalRedirectUri = process.env.GOOGLE_REDIRECT_URI;
      ...
      await testApiHandler({
        ...
        test: async ({ fetch }) => {
          ...
          expect(url.searchParams.get('redirect_uri')).toBe('https://sokopath.com/api/auth/google/callback');
        },
      });
