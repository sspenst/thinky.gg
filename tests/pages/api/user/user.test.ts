  ...
  test('Getting a valid user all the fields except password', async () => {
    await testApiHandler({
      ...
      test: async ({ fetch }) => {
        ...
        expect(keys).toMatchObject([ '__v', '_id', 'config', 'disableAfterLevelPopup', 'disableConfetti', 'disableStreakPopup', 'disallowedEmailNotifications', 'disallowedPushNotifications', 'email', 'emailConfirmed', 'emailDigest', 'lastGame', 'last_visited_at', 'multiplayerProfile', 'name', 'notifications', 'roles', 'ts' ]);
