  ...
  test('Getting a valid user all the fields except password', async () => {
    await testApiHandler({
      ...
      test: async ({ fetch }) => {
        ...
        expect(keys).toMatchObject([ '__v', '_id', 'calcRankedSolves', 'calc_levels_created_count', 'calc_records', 'chapterUnlocked', 'config', 'email', 'last_visited_at', 'multiplayerProfile', 'name', 'notifications', 'roles', 'score', 'ts' ]);
